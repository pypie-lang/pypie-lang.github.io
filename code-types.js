...
const softmaxBlock = {
    kind: "Block",
    body: [
        noWrap(funcDef("softmax", [arg("x", tensorType("float", [typeN]), "Tensor[float][[n]]")], tensorType("float", [typeN]), [
            noWrap(assign(varId("x_exp", "Tensor[float][[n]]"), call(attr(varId("x", "Tensor[float][[n]]"), plainId("exp", "Tensor[float][[n]]")), []))),
            noWrap(ret(binOp(varId("x_exp", "Tensor[float][[n]]"), "/", call(attr(varId("x_exp", "Tensor[float][[n]]"), plainId("sum", "float")), [number(0)])))),
        ], { typeParams: [typeN] })),
    ],
};
const crossEntropyBlock = {
    kind: "Block",
    body: [
        noWrap(funcDef("cross_entropy", [
            arg("ys_pred", tensorType("float", [typeM, typeN]), "Tensor[float][[m, n]]"),
            arg("ys", tensorType("int", [typeM]), "Tensor[int][[m]]"),
        ], typeId("float"), [
            noWrap(assign(varId("target_probs", "Tensor[float][[m]]"), listComp(subscript(varId("probs", "Tensor[float][[n]]"), varId("idx", "int")), [varId("probs", "Tensor[float][[n]]"), varId("idx", "int")], call(plainId("zip"), [
                call(fnId("softmax", "Tensor[float][[m]]"), [varId("ys_pred", "Tensor[float][[m, n]]")]),
                varId("ys", "Tensor[int][[m]]"),
            ])))),
            noWrap(ret(unaryOp("-", call(attr(call(attr(varId("target_probs", "Tensor[float][[m]]"), plainId("log", "Tensor[float][[m]]")), []), plainId("avg", "float")), [number(0)])))),
...
const exampleBlock = {
    kind: "Block",
    body: [
        replAssign(varId("pred", "Tensor[float][[2, 3]]"), call(typeId("Tensor"), [
            listExpr([
                listExpr([number(2.0), number(5.0), number(-6.0)]),
                listExpr([number(1.0), number(3.0), number(5.1)]),
            ]),
        ])),
        replAssign(varId("indices", "Tensor[int][[2]]"), call(typeId("Tensor"), [listExpr([number(0), number(2)])])),
...
const printTypeListItems = (builder, items = [], options = {}) => {
    const shouldWrapItems = !options?.noWrap && Boolean(options?.multiline) && items.length > 0;
    if (shouldWrapItems) {
        builder.newline();
        builder.indent();
...
const estimateTypeListInlineLength = (items = []) => items.reduce((sum, item, index) => sum + estimateTypeLength(item) + (index < items.length - 1 ? 2 : 0), 0);
const printType = (builder, node, options = {}) => {
    ...
    if (node.kind === "TypeList") {
        const shouldWrapTypeList = !forceNoWrap &&
    ...
    if (node.kind === "TypeSubscript") {
        const shouldWrapSubscriptIndex = !forceNoWrap &&
            (resolvedOptions?.multiline === true ||
                (resolvedOptions?.multiline === "auto" &&
...
const printExpr = (builder, node, parentPrecedence = 0, options = {}) => {
    ...
    switch (node.kind) {
        ...
        case "Call": {
            ...
            const shouldWrapArgs = !forceNoWrap &&
                node.args.length > 0 &&
        ...
        case "List": {
            ...
            const shouldWrapElements = !forceNoWrap &&
        ...
        case "BoolOp": {
            ...
            const shouldBreakClauses = !forceNoWrap &&
                node.op === "and" &&
        ...
        case "IfExpr": {
            ...
            const shouldBreakBranches = !forceNoWrap &&
