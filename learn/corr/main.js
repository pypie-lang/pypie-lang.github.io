(() => {
    ...
    const identifier = (name, role, type) => ({
        kind: "Identifier",
        name,
        role,
        ...(type === undefined ? {} : { type }),
    ...
    const listCompExpr = (elt, target, iter) => ({
        kind: "ListComp",
        elt,
        target,
        iter,
    });
    const assignStmt = (target, value) => ({
        kind: "Assign",
        target,
        value,
    });
    const returnStmt = (value) => ({ kind: "Return", value });
    const argNode = (name, annotation, type) => ({
        kind: "Arg",
        name: varIdentifier(name, type),
        annotation,
    });
    const tensorType = (scalarType, shapeItems) => ({
        kind: "TypeSubscript",
        base: {
            kind: "TypeSubscript",
    ...
    const floatTensorType = (shapeItems) => tensorType(typeIdentifier("float"), shapeItems);
    const functionDef = (name, typeParams, args, returns, body) => ({
        kind: "FunctionDef",
    ...
    const codeBlock = (body) => ({ kind: "Block", body });
    const classDef = (name, typeParams, body, decorator) => ({
        kind: "ClassDef",
    ...
    const annAssign = (target, annotation) => ({
        kind: "AnnAssign",
        target,
        annotation,
    });
    const attrExpr = (value, name, type) => ({
        kind: "Attribute",
        value,
        attr: identifier(name, "plain", type),
    });
    const subscriptExpr = (value, index) => ({
        kind: "Subscript",
        value,
        index,
    });
    const sliceExpr = (start, end, step) => ({
        kind: "Slice",
        start,
        end,
        ...(step === undefined ? {} : { step }),
    ...
    const pad1dLengthExpr = () => binOp(binOp(numberLiteral(2), "*", varIdentifier("padding", "int")), "+", varIdentifier("w", "int"));
    const pad1dDefinitionBlock = codeBlock([
        functionDef("pad1d", [typeIdentifier("T"), ...intTypeParams("w")], [
            argNode("xs", tensorType(typeIdentifier("T"), [varIdentifier("w", "int")]), "Tensor[T][[w]]"),
            argNode("padding", typeIdentifier("int"), "int"),
        ], tensorType(typeIdentifier("T"), [pad1dLengthExpr()]), [
            assignStmt(varIdentifier("side_padding", "Tensor[T][[padding]]"), callExpr(fnIdentifier("replicate", "Tensor[T][[padding]]"), [
                varIdentifier("padding", "int"),
                numberLiteral(0),
            ])),
            returnStmt(callExpr(fnIdentifier("concat", "Tensor[T][[2 * padding + w]]"), [
                callExpr(fnIdentifier("concat", "Tensor[T][[padding + w]]"), [
    ...
    const pad2dHeightExpr = () => binOp(binOp(numberLiteral(2), "*", varIdentifier("padding0", "int")), "+", varIdentifier("h", "int"));
    const pad2dWidthExpr = () => binOp(varIdentifier("w", "int"), "+", binOp(numberLiteral(2), "*", varIdentifier("padding1", "int")));
    const pad2dDefinitionBlock = codeBlock([
        functionDef("pad2d", [typeIdentifier("T"), ...intTypeParams("h", "w")], [
            argNode("xs", tensorType(typeIdentifier("T"), [
                varIdentifier("h", "int"),
                varIdentifier("w", "int"),
            ]), "Tensor[T][[h, w]]"),
            ...
            assignStmt(varIdentifier("padded_rows", "Tensor[T][[h, w + 2 * padding1]]"), listCompExpr(callExpr(fnIdentifier("pad1d", "Tensor[T][[w + 2 * padding1]]"), [
                varIdentifier("row", "Tensor[T][[w]]"),
                varIdentifier("padding1", "int"),
            ]), [varIdentifier("row", "Tensor[T][[w]]")], varIdentifier("xs", "Tensor[T][[h, w]]"))),
            assignStmt(varIdentifier("zero_row", "Tensor[T][[w + 2 * padding1]]"), callExpr(fnIdentifier("replicate", "Tensor[T][[w + 2 * padding1]]"), [pad2dWidthExpr(), numberLiteral(0)])),
            assignStmt(varIdentifier("top_bottom", "Tensor[T][[padding0, w + 2 * padding1]]"), callExpr(fnIdentifier("replicate", "Tensor[T][[padding0, w + 2 * padding1]]"), [
                varIdentifier("padding0", "int"),
                varIdentifier("zero_row", "Tensor[T][[w + 2 * padding1]]"),
            ])),
            returnStmt(callExpr(fnIdentifier("concat", "Tensor[T][[2 * padding0 + h, w + 2 * padding1]]"), [
                callExpr(fnIdentifier("concat", "Tensor[T][[padding0 + h, w + 2 * padding1]]"), [
                    varIdentifier("top_bottom", "Tensor[T][[padding0, w + 2 * padding1]]"),
                    varIdentifier("padded_rows", "Tensor[T][[h, w + 2 * padding1]]"),
                ]),
                varIdentifier("top_bottom", "Tensor[T][[padding0, w + 2 * padding1]]"),
            ])),
        ]),
    ...
    const dot2dArgs = () => [
        argNode("s", floatTensorType([intIdentifier("m"), intIdentifier("n")]), "Tensor[float][[m, n]]"),
        argNode("p", floatTensorType([intIdentifier("m"), intIdentifier("n")]), "Tensor[float][[m, n]]"),
    ];
    const dot2dIncompleteDefinitionBlock = codeBlock([
        functionDef("dot2d", intTypeParams("m", "n"), dot2dArgs(), typeIdentifier("float"), [exprStmt(ellipsisExpr())]),
    ]);
    const dot2dDefinitionBlock = codeBlock([
        functionDef("dot2d", intTypeParams("m", "n"), dot2dArgs(), typeIdentifier("float"), [
    ...
    const corr2dMultiInDefinitionBlock = codeBlock([
        functionDef("corr2d_multi_in", intTypeParams("c", "h", "w", "m", "n"), [
    ...
    const corr2dMultiInOutFunctionBlock = codeBlock([
        functionDef("corr2d_multi_in_out", intTypeParams("o", "i", "h", "width", "m", "n"), [
            argNode("s", floatTensorType([intIdentifier("i"), intIdentifier("h"), intIdentifier("width")]), "Tensor[float][[i, h, width]]"),
            argNode("w", floatTensorType([
                intIdentifier("o"),
                intIdentifier("i"),
                intIdentifier("m"),
                intIdentifier("n"),
    ...
    const lenetDefinitionBlock = codeBlock([
        ...
        { kind: "BlankLine" },
        {
            ...
            body: [
                {
                    ...functionDef("predict", intTypeParams("h", "w"), [
                        argNode("x", floatTensorType([numberLiteral(1), intIdentifier("h"), intIdentifier("w")]), "Tensor[float][[1, 28, 28]]"),
                        argNode("params", genericType("Tuple", [
                            floatTensorType([
                                numberLiteral(6),
                                numberLiteral(1),
                                numberLiteral(5),
                        ...
                        assignStmt(varIdentifier("layer1", "Tensor[float][[6, 28, 28]]"), callExpr(fnIdentifier("larger", "Tensor[float][[6, 28, 28]]"), [
                        ...
                        assignStmt(varIdentifier("layer2", "Tensor[float][[6, 14, 14]]"), callExpr(fnIdentifier("pool2d", "Tensor[float][[6, 14, 14]]"), [
                        ...
                        assignStmt(varIdentifier("layer3", "Tensor[float][[16, 10, 10]]"), callExpr(fnIdentifier("larger", "Tensor[float][[16, 10, 10]]"), [
                        ...
                        assignStmt(varIdentifier("layer4", "Tensor[float][[16, 5, 5]]"), callExpr(fnIdentifier("pool2d", "Tensor[float][[16, 5, 5]]"), [
    ...
    render({
        ...
        dialog: [
            ...
            {
                ...message("D", "The score is the dot product between the pattern and the segment, which must have the same shape."),
            ...
            message("W", "The signal `s` and the pattern `p` are rank-1 tensors, but they may have different shapes. " +
            ...
            {
                ...message("W", "It returns `2 * padding` additional elements compared to vanilla `corr1d`."),
            ...
            {
                ...message("D", "Next, let's make `corr1d` more flexible.\n" +
                ...
                buildCodeBlock: (_ast) => seqDefinitionBlock,
            ...
            {
                ...message("W", "`corr1d_stride` should return `(w - n + stride) / stride` elements."),
                codeLabel: "`corr1d_stride` and `corr1d_padded_stride` definitions",
                buildCodeBlock: (_ast) => corr1dStridePairDefinitionBlock,
            },
            {
                ...message("D", "Excellent! We are ready to detect patterns in images.\n" +
                ...
                buildCodeBlock: (_ast) => corr2dIncompleteDefinitionBlock,
            ...
            {
                ...message("W", "How about this? It pads every row left and right, then adds the top and bottom rows with `replicate` and `concat`."),
            ...
            },
            {
                ...
                buildCodeBlock: (_ast) => dot2dIncompleteDefinitionBlock,
            ...
            },
            {
                ...
                buildCodeBlock: (_ast) => dot2dDefinitionBlock,
                textAfterCode: "We multiply the two tensors element-wise, then collapse both dimensions with `sum(0, 1)`."
            },
            {
                ...message("D", "Exactly. Now we can finish `corr2d`, and make `corr2d_with_bias` the small bias wrapper."),
                codeLabel: "`corr2d` and `corr2d_with_bias` definitions",
                buildCodeBlock: (_ast) => corr2dPairDefinitionBlock,
            },
            ...
            message("W", "Interesting. Could we see the functions?"),
            {
                ...message("D", "`corr2d_multi_in` expects rank-3 tensors for `s` and `p`--one pattern for each input channel."),
                codeLabel: "`corr2d_multi_in` definition",
                buildCodeBlock: (_ast) => corr2dMultiInDefinitionBlock,
                textAfterCode: "In the returned value, what's the type of calling `corr2d`?"
            },
            {
                ...message("W", "Calling `corr2d` on rank-3 tensors generates a rank-3 tensor."),
                codeLabel: "`corr2d` return type",
                buildCodeBlock: (_ast) => corr2dMultiInReturnTypeBlock,
                textAfterCode: "But the returned value should be rank-2--does `sum(0)` remove `c`?"
            },
            message("D", "Yes. We can specify dimensions for `sum`. Here, `sum` collapses the first dimension, `c`, producing the expected rank-2 tensor."),
            message("W", "That is very handy!"),
            {
                ...message("D", "Next, for multiple output channels, we pass the output-channel weights and biases separately."),
                codeLabel: "`corr2d_multi_in_out` definition",
                buildCodeBlock: (_ast) => corr2dMultiInOutDefinitionBlock,
            },
            message("W", "I see. `w` stores `o` patterns, and `b` stores one bias for each output channel.\n" +
                "And we can directly reuse `corr2d_multi_in`."),
            message("D", "`corr2d_multi_in_out` detects patterns at pixel-level granularity.\n" +
            ...
            message("W", "How do we reduce the impact of exact locations?"),
            {
                ...message("D", "We shrink the image with !!pooling!!."),
                codeLabel: "`mean2d` and `pool2d` definitions",
                buildCodeBlock: (_ast) => poolingDefinitionBlock,
                textAfterCode: "Here, we reduce each `h * w` block to one value by taking the average of all elements in the block."
            },
            message("W", "So the precise locations matter less!"),
            {
                ...message("D", "We now have most of the ingredients for the famous LeNet.\n" +
                    "It takes images of size `28 * 28`, with `1` input channel,\n" +
                    "and generates likelihood scores for `10` items."),
                codeLabel: "`LeNet` definition",
                buildCodeBlock: (_ast) => lenetDefinitionBlock,
            ...
            message("W", "Does `larger(x, y)` pick the larger number between `x` and `y`? Why do we need these comparisons?"),
            ...
            message("D", "Right. Our model can only detect patterns--it does not reason about them yet.\n" +
                "We will complete LeNet in the next chapter, and run it on Fashion-MNIST!"),
            message("W", "This is getting exciting!")
        ],
    });
})();
