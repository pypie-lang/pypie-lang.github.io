(() => {
    ...
    render({
        id: "prelude",
        dialog: [
            message("D", "LeNet works like a detective who is specialized in pictures. " +
            ...
            message("W", "What's the name of our new detective?"),
            message("D", "Let's call it the LittleTransformer.\n" +
            ...
            message("W", "Attention sounds like the model is looking around before making up its mind."),
            message("D", "Suppose we are reading the sentence\n" +
            ...
            message("W", "Let's define a function to compute scores!"),
            message("D", "To compare tokens, we first turn each one into a vector of numbers called an !!embedding!!.\n" +
            ...
            message("W", "You just said `token`.\n" +
                "Is a token the same thing as a word?"),
            message("D", "A token is one unit that the model reads.\n" +
            ...
            message("W", "Then, attention is a function that takes in embeddings and generates scores?"),
            {
                ...message("D", "Yes.\n" +
                ...
                buildCodeBlock: (_ast) => selfAttentionDefinitionBlock,
                textAfterCode: "`k.permute(1, 0)` swaps the two dimensions of `k`.\n" +
            ...
            message("W", "Hmm... `d` is like the input channels of LeNet and `n` is like the output channels?\n" +
                "Why is `attention` still missing a line?"),
            {
                ...message("D", "Channels are a useful analogy, and we will return to it soon.\n" +
                ...
                buildCodeBlock: (_ast) => causalMaskDefinitionBlock,
                textAfterCode: "`causal_mask` keeps the past and replaces the future with small numbers.\n" +
                    "Then `softmax` gives almost zero weight to those future positions."
            },
            {
                ...message("W", "Then the complete `attention` should look like this."),
            ...
            message("W", "I see! Can we simply add an extra dimension for `q`, `k`, and `v`?"),
            {
                ...
                buildCodeBlock: (_ast) => multiHeadsAttentionDefinitionBlock,
            },
            message("W", "Now we have the clue-collector, how about the decision-maker?"),
            {
            ...
            { ...message("D", "Yes, `4` works well in practice.\n" +
                ...
                buildCodeBlock: (_ast) => blockDefinitionV1Block,
            },
            message("W", "The `block` adds the attention back to `x`. So it keeps refining what it has learned.\n" +
            ...
            },
            {
                ...
                buildCodeBlock: (_ast) => blockDefinitionV2Block,
            },
            {
                ...message("D", "Good. Now we can draft `predict`.\n" +
                ...
                buildCodeBlock: (_ast) => littleTransformerClassBlock,
            },
            message("W", "There is a little gap: the inputs are tokens, but `block` needs their embeddings."),
            {
                ...message("D", "Exactly. The next step is to turn token IDs into embeddings with `emb`.\n" +
                ...
                buildCodeBlock: (_ast) => embDefinitionBlock,
                textAfterCode: "Next we give two embeddings to `predict`."
            },
            message("W", "Which two?"),
            {
                ...message("D", "One for meanings and one for positions. These two embeddings are complementary. Here we simply add them together."),
                codeLabel: "`LittleTransformer` with complete `predict`",
                buildCodeBlock: (_ast) => littleTransformerCompletePredictBlock,
                textAfterCode: "Now `predict` is complete. Let's define `loss`. What's the type of `ys_pred`?"
            },
            message("W", "`loss` takes in a batch of predictions. So `ys_pred` should be some `Tensor[float][[b, l, vocab_size]]`."),
            {
                ...message("D", "Right, here's our definition of `loss`. For each input, `ys_pred` contains the scores for all tokens, " +
                ...
                buildCodeBlock: (_ast) => littleTransformerLossBlock,
            },
            message("W", "We have `predict` and `loss`. How about `update`?"),
            {
                ...message("D", "We use our old RMS `update` to stabilize gradients. Now our LittleTransformer is complete!"),
