(() => {
    ...
    render({
        ...
        dialog: [
            ...
            message(
                "W",
                "`[2, 3]` is a `List[int]` that describes the !!shape!! of `Tensor([[1, 2, 3], [4, 5, 6]])`, since the outer layer contains two `List[int]`s and each inner layer contains three `int`s?",
            ),
            message(
                "D",
                "Not quite, but close.\n" +
