(() => {
    ...
    const buildConcat11Example = (ast: AstApi) => {
        const m = ast.varId("m", "int");
            ...
            ast.funcDef(
                ...
                [
                    ast.noWrap(
                        ast.ret(
                            ast.listComp(
                                ast.call(ast.plainId("concat"), [ast.varId("x"), ast.varId("y")]),
                                [ast.tupleExpr([ast.varId("x"), ast.varId("y")])],
                                ast.call(ast.plainId("zip"), [ast.varId("xs"), ast.varId("ys")])
                            )
                        )
                    ),
                ],
