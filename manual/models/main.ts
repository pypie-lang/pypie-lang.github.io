(() => {
    ...
    const buildModelContract = (ast: AstApi) => {
    ...
    const buildHighLevelInterfaces = (ast: AstApi) => {
        const x = ast.typeId("X");
        const yp = ast.typeId("Yp");
        const y = ast.typeId("Y");
        const p = ast.typeId("P");
    ...
    const buildLineModel = (ast: AstApi) => {
        const float32 = ast.typeId("float32");
        const n = ast.varId("n", "int64");
        const lineParams = ast.typeSubscript(
            ast.typeId("Tuple"),
            ast.typeList([float32, float32])
        );
                            ...
                            [
                                ast.noWrap(
                                    ast.ret(
                                        ast.binOp(
                                            ast.binOp(
                                                ast.subscript(lineParamsId(), ast.number(0)),
                                                "*",
                                                ast.varId("x", "float32")
                                            ),
                                            "+",
                                            ast.subscript(lineParamsId(), ast.number(1))
                                        )
                                    )
                                ),
                            ],
                                    ...
                                    ast.ret(
                                        ast.call(
                                            ast.attr(
                                                ast.binOp(
                                                    ast.binOp(
                                                        ast.varId("ys_pred", lineTensorType),
                                                        "-",
                                                        ast.varId("ys", lineTensorType)
                                                    ),
                                                    "**",
                                                    ast.number(2)
                                                ),
    ...
    const buildLineAdamModel = (ast: AstApi) => {
        const t = ast.typeId("T");
        const n = ast.varId("n", "int64");
        ...
        const stateType = ast.typeSubscript(
            ast.typeId("tuple"),
            ast.typeList([t, t, t])
        );
        const stateTypeText = "tuple[T, T, T]";

        const stateItem = (index: number) =>
            ast.subscript(ast.varId("s", stateTypeText), ast.number(index));
                            ...
                            [
                                ast.noWrap(
                                    ast.ret(
                                        ast.binOp(
                                            ast.binOp(
                                                ast.attr(ast.varId("p", paramsTypeText), "w"),
                                                "*",
                                                ast.varId("x", "T")
                                            ),
                                            "+",
                                            ast.attr(ast.varId("p", paramsTypeText), "b")
                                        )
                                    )
                                ),
                            ],
                                    ...
                                    ast.ret(
                                        ast.call(
                                            ast.attr(
                                                ast.binOp(
                                                    ast.binOp(
                                                        ast.varId("ys_pred", tensorTypeText),
                                                        "-",
                                                        ast.varId("ys", tensorTypeText)
                                                    ),
                                                    "**",
                                                    ast.number(2)
                                                ),
