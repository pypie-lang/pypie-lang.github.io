(() => {
    ...
    const buildStrictConversionErrors = (ast: AstApi) => {
        ...
        const commentReturn = (text: string, left: unknown, right: unknown) => [
            ast.comment(text),
        ...
        return ast.block([
            ast.noWrap(
                ast.funcDef(
                    ...
                    x,
                    commentReturn("`X` != `Y`", ast.varId("x", "X"), ast.varId("y", "Y")),
                    { typeParams: [x, y] }
                )
            ),
            blankLine(),
            ast.noWrap(
                ast.funcDef(
                    ...
                    float32,
                    commentReturn("`X` != `float32`", ast.varId("x", "X"), ast.varId("y", "float32")),
                    { typeParams: [x] }
                )
            ),
            blankLine(),
    ...
    const buildStrictConversionCasts = (ast: AstApi) => {
        const x = ast.typeId("X");
        const y = ast.typeId("Y");
        const float32 = ast.typeId("float32");
        const float16 = ast.typeId("float16");
        ...
        return ast.block([
            ast.noWrap(
                ast.funcDef(
                    ...
                    x,
                    addReturn(ast.varId("x", "X"), cast(ast.varId("y", "Y"), x)),
                    { typeParams: [x, y] }
                )
            ),
            blankLine(),
            ast.noWrap(
                ast.funcDef(
                    ...
                    float32,
                    addReturn(cast(ast.varId("x", "X"), float32), ast.varId("y", "float32")),
                    { typeParams: [x] }
                )
            ),
            blankLine(),
            ast.noWrap(
                ast.funcDef(
                    ...
                    float32,
                    addReturn(ast.varId("x", "float32"), cast(ast.varId("y", "float16"), float32)),
                    { typeParams: [x] }
                )
            ),
        ]);
    ...
    const buildLiteralIsFlexible = (ast: AstApi) => {
        const x = ast.typeId("X");
        ...
        return ast.block([
            ast.noWrap(
                ast.funcDef(
                    ...
                    [
                        ast.noWrap(
                            ast.ret(ast.binOp(ast.varId("x", "X"), "+", ast.number(0.1)))
                        ),
                    ],
                    { typeParams: [x] }
                )
