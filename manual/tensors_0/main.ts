(() => {
    ...
    const buildTensorExample = (ast: AstApi) => {
        ...
        return ast.block([
            ast.noWrap(
                ast.funcDef(
                    "column_energy",
                    [ast.arg("xs", ast.tensorType("float", [m, n]))],
                    ast.tensorType("float", [n]),
                    [
                        ast.noWrap(
                            ast.assign(
                                ast.varId("squared"),
                                ast.binOp(ast.varId("xs"), "*", ast.varId("xs"))
                            )
                        ),
                        ast.noWrap(
                            ast.ret(
                                ast.call(ast.attr(ast.varId("squared"), "sum"), [ast.number(0)])
                            )
                        ),
                    ],
                    { typeParams: [m, n] }
