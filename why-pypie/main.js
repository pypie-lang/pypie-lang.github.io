(() => {
    ...
    const normalizeBenchmarkCell = (value) => {
    ...
    const parseBenchmarkColumns = (value) => {
    ...
    const parseBenchmarkTableData = (value) => {
        ...
        const columnGroups = Array.isArray(value.columnGroups)
            ...
            : [];
        const fallbackColumns = columnGroups.length === 0 && Array.isArray(value.columns) ? parseBenchmarkColumns(value.columns) : [];
        const normalizedColumnGroups = columnGroups.length > 0 ? columnGroups : fallbackColumns.length > 0 ? [{ label: "", columns: fallbackColumns }] : [];
        ...
        const rowHeaderLabel = typeof value.rowHeaderLabel === "string" && value.rowHeaderLabel.trim().length > 0
    ...
    const ensureBenchmarkTableStyles = () => {
    ...
    const getEmbeddedBenchmarkTableData = () => {
    ...
    const eyebrow = document.querySelector("[data-manual-eyebrow]");
    if (eyebrow) {
        eyebrow.textContent = "";
        eyebrow.hidden = true;
    }
